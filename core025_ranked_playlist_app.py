...
def load_member_score_overlay_df(overlay_path: str = "", overlay_file=None) -> Tuple[pd.DataFrame, Dict[str, object]]:
    ...
    for c in ["when_prev_date_no9_row", "when_seed_has9", "when_strong_repeat", "when_strong_switch", "rank_min", "rank_max", "enabled"]:
        df[c] = pd.to_numeric(df[c], errors="coerce")
    for c in ["delta_0025", "delta_0225", "delta_0255"]:
        df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0.0)
    for c in ["when_row_memory_tag", "when_last_winner", "when_base_top1", "when_base_top2", "when_base_top3", "rule_id", "note"]:
...
def _apply_row_memory_context_to_ranked(
    ...
    for idx, row in out.iterrows():
        ...
        apply_here = (not no9_only) or (not bool(seed_has9_flag))
        if apply_here:
            score_delta, action = _row_memory_score_adjustments(
                row=row,
                row_profile=profile,
                current_order=current_order,
                prev_row_flag=prev_row_flag,
...
with st.sidebar.expander("Mined member-layer packs (elims / 1-miss downranks / rescues)", expanded=False):
    ...
    st.caption("Overlay CSV columns: rule_id, enabled, when_row_memory_tag, when_last_winner, when_prev_date_no9_row, when_seed_has9, when_strong_repeat, when_strong_switch, when_base_top1, when_base_top2, when_base_top3, rank_min, rank_max, delta_0025, delta_0225, delta_0255, note")
