...
def _row_memory_score_adjustments(row_profile: Optional[Dict[str, object]]) -> Tuple[Dict[str, float], str]:
    ...
    # Active tested live rule: strong-switch row + last winner 25 => real numeric penalty on 25.
    if row_profile.get("strong_switch") and last_member == "0025":
        delta["0025"] -= 2.10
        actions.append("SW25_SCORE_NEG")

    # Active tested live rule: strong-repeat row => real numeric bonus to the last winner.
    if row_profile.get("strong_repeat") and last_member in delta:
        delta[last_member] += 2.10
        actions.append(f"{row_profile.get('tag', 'REP')}_SCORE_BOOST")
...
def _apply_row_memory_context_to_ranked(
    ...
    for idx, row in out.iterrows():
        ...
        if apply_here:
            score_delta, action = _row_memory_score_adjustments(profile)
