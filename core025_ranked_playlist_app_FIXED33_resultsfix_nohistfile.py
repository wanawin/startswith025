...
def extract_4digits(result_text: str) -> Optional[str]:
    """Extract a 4-digit Pick4-style result from a Lottery Post Results cell.
    ...
    Attempts to ignore bonus-ball annotations (Fireball/Wild Ball/etc.) so we don't accidentally
    ingest the bonus digit as part of the 4-digit result.
    """
    ...
    # Remove explicit bonus annotations (only when tied to a keyword).
    cleaned = re.sub(
        r"[,;\s]*(fireball|wild\s*ball|wildball|bonus|extra|multiplier)\s*[:=]?\s*\d+",
        "",
        raw,
        flags=re.IGNORECASE,
    )

    m = _DIG4_RE.search(cleaned)
    if m:
        return "".join(m.groups())

    digs = re.findall(r"\d", cleaned)
    if len(digs) >= 4:
        return "".join(digs[:4])
