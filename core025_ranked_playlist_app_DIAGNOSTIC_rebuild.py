...
# -----------------------------
# Input signature (auto-invalidate results when inputs change)
# -----------------------------
import hashlib
...
def _file_sig_from_upload(u) -> str:
    try:
        b = u.getvalue()
        return f"upload:{_bytes_sig(b)}:{len(b)}"
    except Exception:
        return "upload:ERR"

def _file_sig_from_path(p: str) -> str:
    try:
        with open(p, "rb") as f:
            b = f.read()
        return f"path:{_bytes_sig(b)}:{len(b)}"
    except Exception:
        return f"path:MISS:{p}"

def compute_inputs_signature(hist_up, weights_up, tiepack_up, default_hist_path, default_weights_path, default_tiepack_path,
                             gate_no9, max_rules, min_support, tierA_gap, tierB_gap_low, tierB_gap_high) -> str:
    sig_parts = [
        _file_sig_from_upload(hist_up) if hist_up is not None else _file_sig_from_path(default_hist_path),
        _file_sig_from_upload(weights_up) if weights_up is not None else _file_sig_from_path(default_weights_path),
        _file_sig_from_upload(tiepack_up) if tiepack_up is not None else _file_sig_from_path(default_tiepack_path),
        f"gate_no9={int(gate_no9)}",
        f"max_rules={int(max_rules)}",
        f"min_support={int(min_support)}",
        f"tierA_gap={int(tierA_gap)}",
        f"tierB_gap_low={int(tierB_gap_low)}",
        f"tierB_gap_high={int(tierB_gap_high)}",
    ]
    return " | ".join(sig_parts)

def _read_bytes_from_path(p: str) -> bytes:
    with open(p, "rb") as f:
        return f.read()

def describe_source(upload, default_path: str):
    """
    Returns (label, bytes, size, short_hash)
    """
    if upload is not None:
        b = upload.getvalue()
        return ("uploaded", b, len(b), _bytes_sig(b))
    try:
        b = _read_bytes_from_path(default_path)
        return (default_path, b, len(b), _bytes_sig(b))
    except Exception:
        return (f"missing:{default_path}", b"", 0, "NA")
...
# -----------------------------
# Session state (fixes download button not clicking)
# -----------------------------
if "ranked_df" not in st.session_state:
    st.session_state["ranked_df"] = None
if "ranked_cols" not in st.session_state:
    st.session_state["ranked_cols"] = None
...
st.caption("Download buttons stay clickable because results persist in session state (no re-run reset).")
...
with st.sidebar:
    ...
    reset_btn = st.button("Reset results (clears current playlist)", use_container_width=True)
    force_rebuild = st.checkbox("Force rebuild (ignore cached results)", value=False)



# Auto-invalidate results when inputs/settings change (prevents “same list every time”)
current_sig = compute_inputs_signature(
    hist_up, weights_up, tiepack_up,
    default_hist_path, default_weights_path, default_tiepack_path,
    gate_no9, max_rules, min_support, tierA_gap, tierB_gap_low, tierB_gap_high
)
prev_sig = st.session_state.get("inputs_sig")

if "reset_btn" in locals() and reset_btn:
    st.session_state["ranked_df"] = None
    st.session_state["ranked_cols"] = None
    st.session_state["meta"] = {}
    st.session_state["inputs_sig"] = current_sig
    st.toast("Results cleared. Click 'Build ranked playlist' to rebuild.", icon="🧹")
...
with st.expander("Debug: input signature (changes when your history/weights/tie-pack/settings change)"):
    st.code(current_sig, language="text")

    # Also show what files the app is actually using (prevents “it didn’t change” mystery)
    h_label, h_bytes, h_size, h_hash = describe_source(hist_up, default_hist_path)
    w_label, w_bytes, w_size, w_hash = describe_source(weights_up, default_weights_path)
    t_label, t_bytes, t_size, t_hash = describe_source(tiepack_up, default_tiepack_path)
    st.write("**History source:**", h_label, "| size:", h_size, "| hash:", h_hash)
    st.write("**Weights source:**", w_label, "| size:", w_size, "| hash:", w_hash)
    st.write("**Tie-pack source:**", t_label, "| size:", t_size, "| hash:", t_hash)
...
if "force_rebuild" not in locals():
    force_rebuild = False
...
if run_btn:
    with st.spinner("Running… computing from your real history + weights + tie-pack"):
        ...
        # Quick sanity stats on the loaded history (so you can confirm the new file is actually different)
        st.write(f"Loaded history rows (raw): {len(hist_raw):,} from {hist_src}")
        ...
        st.write(f"025-core hit-events found (pre-gate): {len(core):,}")
        ...
        st.write(f"025-core hit-events after gate_no9={gate_no9}: {len(core):,}")
        ...
        show_cols = ["stream","date","seed_digits","winner","final_pick","score1","score2","score3","gap12",
                     "top2_recommended","dead_tie_preCR","tie_fired","tier","why"]

        # Persist results so download button clicks don't "lose" the table on rerun
        st.session_state["ranked_df"] = ranked
        st.session_state["inputs_sig"] = current_sig
        st.session_state["ranked_cols"] = show_cols
        st.session_state["meta"] = {
            "history": hist_src, "weights": w_src, "tiepack": tp_src,
            "gate_no9": gate_no9, "max_rules": max_rules, "min_support": min_support,
            "tierA_gap": tierA_gap, "tierB_gap_low": tierB_gap_low, "tierB_gap_high": tierB_gap_high,
            "met_rows": met_rows, "rules_by_seg": rules_by_seg
        }

# -----------------------------
# Display persisted results (always)
# -----------------------------
ranked = st.session_state.get("ranked_df")
show_cols = st.session_state.get("ranked_cols")
...
st.success(f"Events={len(ranked):,} | Top-1 hits={hits:,} ({acc:.2f}%) | top2_rec={t2n:,} | dead_tie={dtn:,}")
st.caption(f"History: {meta.get('history')} | Weights: {meta.get('weights')} | Tie-pack: {meta.get('tiepack')}")
...
st.download_button(
    "Download ranked playlist (CSV)",
    data=csv_bytes,
    file_name=f"core025_ranked_playlist_{ts}.csv",
    mime="text/csv",
...
lines.append(f"Gate seed_no9={meta.get('gate_no9')} | max_rules={meta.get('max_rules')} | min_support={meta.get('min_support')}")
lines.append("")
...
st.download_button(
    "Download ranked playlist (TXT)",
    data=txt_bytes,
    file_name=f"core025_ranked_playlist_{ts}.txt",
    mime="text/plain",
    use_container_width=True,
    key="dl_txt"
)

with st.expander("Per-member override stacks (impact)"):
    met_df = pd.DataFrame(meta.get("met_rows", []))
    st.dataframe(met_df, use_container_width=True)

with st.expander("Show rule stacks"):
    rules_by_seg = meta.get("rules_by_seg", {})
    for seg in MEMBERS:
        st.markdown(f"### Top1 segment = `{seg}` (applies only when Top-2 is recommended)")
        rules = rules_by_seg.get(seg, [])
        if not rules:
