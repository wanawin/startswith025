...
def seed_unique_bool(seed4: str) -> bool:
    # IMPORTANT: seed_unique==1 means "all 4 digits unique" (no repeats)
    try:
        return len(set(str(seed4))) == 4
    except Exception:
        return False

def seed_even_cnt(seed4: str) -> int:
    try:
        return sum(1 for ch in str(seed4) if ch.isdigit() and int(ch) % 2 == 0)
    except Exception:
        return 0

def seed_has_pair(seed4: str) -> bool:
    try:
        s = str(seed4)
        return any(s.count(ch) >= 2 for ch in set(s))
    except Exception:
        return False

def seed_consec_links(seed4: str) -> int:
    # Count of consecutive links among UNIQUE sorted digits (no wraparound)
    try:
        digs = sorted({int(ch) for ch in str(seed4) if ch.isdigit()})
        return sum(1 for a, b in zip(digs, digs[1:]) if b - a == 1)
    except Exception:
        return 0

def seed_mirrorpair_cnt(seed4: str) -> int:
    try:
        digs = {int(ch) for ch in str(seed4) if ch.isdigit()}
        return sum(1 for a,b in _MIRROR_PAIRS if a in digs and b in digs)
    except Exception:
        return 0

def seed_adj_absdiff_sum(seed4: str) -> int:
    # Adjacent positional absdiff sum: |d0-d1| + |d1-d2| + |d2-d3|
    try:
        d = seed_digits(str(seed4))
        return abs(d[0]-d[1]) + abs(d[1]-d[2]) + abs(d[2]-d[3])
    except Exception:
        return 0

def seed_pairwise_absdiff_sum(seed4: str) -> int:
    # Sum of absdiff over all unordered pairs (6 pairs)
    try:
        d = seed_digits(str(seed4))
        s = 0
        for i in range(4):
...
def seed_digit_counts(seed4: str) -> Dict[int, int]:
    ...
    out = {}
    for d in range(10):
        out[d] = int(s.count(str(d)))
    return out

def seed_has_pair_39(seed4: str) -> bool:
    s = str(seed4)
    return ("3" in s and "9" in s) or ("9" in s and "3" in s)

def seed_has_pair_55(seed4: str) -> bool:
    s = str(seed4)
    return s.count("5") >= 2

def seed_has_pair_26(seed4: str) -> bool:
    s = str(seed4)
    return ("2" in s and "6" in s) or ("6" in s and "2" in s)

def seed_has_pair_29(seed4: str) -> bool:
    s = str(seed4)
    return ("2" in s and "9" in s) or ("9" in s and "2" in s)

def seed_has_pair_79(seed4: str) -> bool:
    s = str(seed4)
    return ("7" in s and "9" in s) or ("9" in s and "7" in s)

def compute_features(seed4: str) -> Dict[str, object]:
    s = str(seed4)
    counts = seed_digit_counts(s)
    feats: Dict[str, object] = {
        # Existing fields (DO NOT REMOVE)
        "seed_sum": seed_sum(s),
        "seed_spread": seed_spread(s),
        "seed_absdiff": seed_absdiff(s),
        "seed_has_worstpair_025": seed_has_worstpair_025(s),
        "seed_sum_lastdigit": seed_sum_lastdigit(s),
        "seed_has_9": ("9" in s),

        # New primitives (additive)
        "seed_unique": int(seed_unique_bool(s)),
        "seed_even_cnt": int(seed_even_cnt(s)),
        "seed_has_pair": int(seed_has_pair(s)),
        "seed_consec_links": int(seed_consec_links(s)),
        "seed_mirrorpair_cnt": int(seed_mirrorpair_cnt(s)),
        "seed_adj_absdiff_sum": int(seed_adj_absdiff_sum(s)),
        "seed_pairwise_absdiff_sum": int(seed_pairwise_absdiff_sum(s)),

        # Explicit worstpair flags (optional but useful)
        "seed_has_pair_39": int(seed_has_pair_39(s)),
        "seed_has_pair_55": int(seed_has_pair_55(s)),
        "seed_has_pair_26": int(seed_has_pair_26(s)),
        "seed_has_pair_29": int(seed_has_pair_29(s)),
        "seed_has_pair_79": int(seed_has_pair_79(s)),
    }

    # seed_has{0..9} + seed_cnt{0..9}
    for d in range(10):
        feats[f"seed_cnt{d}"] = int(counts.get(d, 0))
        feats[f"seed_has{d}"] = int(counts.get(d, 0) > 0)
...
with st.sidebar:
    ...
    st.divider()
    st.header("Top1 Support (visual)")
    support_gap12 = st.slider("Top1-only supported if Gap12 ≥", 0.0, 10.0, 1.0, 0.1,
        help="Used only for bolding/SupportReason in LIVE & LAB tables. Does NOT force Top2 unless FlipRec fires.")
    split_seed_has9_tables = st.checkbox("Split seed-has9 streams into a separate table", value=True)
...
# Latest seed per stream
    latest_all = hist_cad.sort_values(["stream", "Draw Date"]).groupby("stream").tail(1).copy()
    latest_all = latest_all.rename(columns={"result": "seed_result", "Draw Date": "seed_date"})
    latest_all["seed_has9"] = latest_all["seed_result"].astype(str).str.contains("9")

    # Gate no9 (locked core gate) — keep the gated rows visible, but don't let them affect the main ranking
    pre_gate_latest = latest_all.copy()
    gated_out_live = pre_gate_latest[pre_gate_latest["seed_has9"]].copy()
    latest = pre_gate_latest[~pre_gate_latest["seed_has9"]].copy() if gate_no9 else pre_gate_latest.copy()

    # Score ALL streams (including seed_has9) so we can show a separate table when needed.
    latest_for_scoring = pre_gate_latest.copy()
...
# Visibility: which streams were excluded by the NO9 gate today
    ...
    rows = []
    for _, r in latest_for_scoring.iterrows():
        seed4 = r["seed_result"]
        ...
        rows.append({
            ...
            # Per-member base scores (so Top1/Top2/Top3 can be audited)
            "Score_0025": float(s["scores"].get("0025", 0.0)),
            "Score_0225": float(s["scores"].get("0225", 0.0)),
            "Score_0255": float(s["scores"].get("0255", 0.0)),

            # Convenience scores/gaps for the chosen ordering
            "Top1Score": float(s["scores"].get(s["top1"], 0.0)),
            "Top2Score": float(s["scores"].get(s["top2"], 0.0)),
            "Top3Score": float(s["scores"].get(s["top3"], 0.0)),
            "Gap12": float(s["scores"].get(s["top1"], 0.0) - s["scores"].get(s["top2"], 0.0)),
            "Gap23": float(s["scores"].get(s["top2"], 0.0) - s["scores"].get(s["top3"], 0.0)),

            # Legacy diagnostics (do not remove)
            "BaseGap(#1-#2)": float(s["base_gap"]),
            "TieFired": int(s["tie_fired"]),
            "DeadTie": int(s["dead_tie"]),
            "ForcedPick": s["forced_pick"] if s["forced_pick"] else "",
            "SeedSum": int(s["features"]["seed_sum"]),
            "SeedSpread": int(s["features"]["seed_spread"]),
            "DrawsSinceLast025": int(r.get("draws_since_last_025", 9999)),
            "Rolling025_30": int(r.get("rolling_025_30", 0)),
            "SeedHas9": int(bool(s["features"].get("seed_has_9", False))),
...
if not out_all.empty:
    out_all["StreamKey"] = out_all["Stream"].apply(canon_stream)

# Separate seed_has9 segment (used for visibility / diagnostics)
if "SeedHas9" in out_all.columns:
    out_has9 = out_all[out_all["SeedHas9"].astype(int) == 1].copy()
else:
    out_has9 = out_all.iloc[0:0].copy()

# Apply NO9 gate to the MAIN ranking only (seed_has9 rows will not affect percentiles/ranks)
if gate_no9 and ("SeedHas9" in out_all.columns):
    out = out_all[out_all["SeedHas9"].astype(int) == 0].copy()
else:
    out = out_all.copy()

if out.empty:
    st.warning("No streams left after gating/filtering. Try turning off gate_no9 or uploading more history.")
else:
        ...
        # --- Top1-only support flag (visual; does NOT auto-add Top2) ---
        _gap12 = pd.to_numeric(out.get("Gap12", 0), errors="coerce").fillna(0.0)
        _seedhas9 = pd.to_numeric(out.get("SeedHas9", 0), errors="coerce").fillna(0).astype(int)

        out["Top1OnlySupported"] = (
            (out.get("FlipRec", 0).astype(int) == 0) &
            (out.get("DeadTie", 0).astype(int) == 0) &
            (out.get("TieFired", 0).astype(int) == 0) &
            (_gap12 >= float(support_gap12)) &
            (_seedhas9 == 0)
        ).astype(int)

        _gap_reason = f"gap12<{support_gap12:.1f}"
        out["SupportReason"] = np.select(
            [
                (_seedhas9 == 1),
                (out.get("DeadTie", 0).astype(int) == 1),
                (out.get("FlipRec", 0).astype(int) == 1),
                (out.get("TieFired", 0).astype(int) == 1),
                (_gap12 < float(support_gap12)),
                (out["Top1OnlySupported"].astype(int) == 1),
            ],
            [
                "seed_has9",
                "dead_tie",
        ...
        # Prepare a separate seed_has9 table (scored, but not affecting the main ranks)
        out_has9_disp = out_has9.copy()
        if split_seed_has9_tables and (not out_has9_disp.empty):
            out_has9_disp = out_has9_disp.sort_values(["StreamScore", "Top1Score", "BaseGap(#1-#2)"], ascending=False).reset_index(drop=True)
            out_has9_disp.insert(0, "Rank", np.arange(1, len(out_has9_disp)+1))

            # Tier within this segment (informational)
            try:
                _qA9 = out_has9_disp["StreamScore"].quantile(0.20)
                _qB9 = out_has9_disp["StreamScore"].quantile(0.50)
                out_has9_disp["Tier"] = out_has9_disp["StreamScore"].apply(lambda s: "A" if s >= _qA9 else ("B" if s >= _qB9 else "C"))
            except Exception:
                out_has9_disp["Tier"] = ""

            out_has9_disp["FlipRec"] = ((out_has9_disp["BaseGap(#1-#2)"] == 0) & (out_has9_disp["TieFired"] == 1) & (out_has9_disp["DeadTie"] == 0)).astype(int)
            out_has9_disp["PlayPick"] = np.where(out_has9_disp["FlipRec"] == 1, "TOP2", "TOP1")
            out_has9_disp["PlayMember"] = np.where(out_has9_disp["FlipRec"] == 1, out_has9_disp["Top2"], out_has9_disp["PredictedMember"])
            out_has9_disp["PlayPlan"] = np.where(out_has9_disp["FlipRec"] == 1, "TOP1+TOP2", "TOP1")
            out_has9_disp["PlayMembers"] = np.where(
                out_has9_disp["FlipRec"] == 1,
                out_has9_disp["PredictedMember"].astype(str) + " + " + out_has9_disp["Top2"].astype(str),
                out_has9_disp["PredictedMember"].astype(str),
            )
            out_has9_disp["PlayCount"] = np.where(out_has9_disp["FlipRec"] == 1, 2, 1).astype(int)

            _gap12_9 = pd.to_numeric(out_has9_disp.get("Gap12", 0), errors="coerce").fillna(0.0)
            _seedhas9_9 = pd.to_numeric(out_has9_disp.get("SeedHas9", 0), errors="coerce").fillna(0).astype(int)
            out_has9_disp["Top1OnlySupported"] = 0
            out_has9_disp["SupportReason"] = np.where(_seedhas9_9 == 1, "seed_has9", "not_supported")
        ...
        st.caption("Full list ranked. Tier A ≈ top 20% by StreamScore; Tier B ≈ next 30%; Tier C ≈ bottom 50% (no cutoff). "
           ...
           "Top1OnlySupported=1 (bold Top1) is a VISUAL support flag: not tie-fired, not dead-tie, Gap12 ≥ support threshold, and seed_has9=0.")


        
        def _style_flip(df):
            # Bold TOP1 when Top1OnlySupported==1; bold play columns (and Top2) when FlipRec==1
            def _row_style(row):
                flip = int(row.get("FlipRec", 0)) == 1
                supported = int(row.get("Top1OnlySupported", 0)) == 1

                styles = ["" for _ in df.columns]
                for i, col in enumerate(df.columns):
                    if flip and col in ("PlayPlan", "PlayMembers", "PlayCount", "PlayMember", "PlayPick", "Top2"):
                        styles[i] = "font-weight:700;"
                    if supported and col in ("PredictedMember", "Top1Score", "PlayPlan", "PlayMembers"):
                        styles[i] = "font-weight:700;"
                return styles
        ...
        st.download_button("Download LIVE ranked streams (CSV)", data=csv_bytes, file_name=f"core025_live_ranked_streams_{datetime.now().strftime('%Y-%m-%d_%H%M')}.csv", mime="text/csv")


# Optional: show seed_has9 segment in a separate table (does not affect the main ranking)
if split_seed_has9_tables and (not out_has9_disp.empty):
    st.divider()
    st.subheader("Seed contains 9 (separate segment)")
    st.caption("Shown for visibility/diagnostics. If NO9 gate is ON, these are GATED OUT from main play. "
               "Even if NO9 gate is OFF, they are kept separate and do not affect the main ranking/percentiles.")

    _render9 = out_has9_disp.copy()
    _cols9 = list(_render9.columns)
    _play_cols = ["PlayPlan", "PlayMembers", "PlayCount", "PlayMember", "PlayPick", "FlipRec", "Top1OnlySupported", "SupportReason"]
    if "PredictedMember" in _cols9:
        for c in _play_cols:
            if c in _cols9:
                _cols9.remove(c)
        insert_at = _cols9.index("PredictedMember") + 1
        for c in reversed([c for c in _play_cols if c in _render9.columns]):
            _cols9.insert(insert_at, c)

    styled9 = _style_flip(_render9[_cols9])
    st.dataframe(styled9, use_container_width=True, height=min(38, len(_render9)) * 26 + 40)

    csv9 = _render9.to_csv(index=False).encode("utf-8")
    st.download_button("Download seed_has9 segment (CSV)", data=csv9,
                       file_name=f"core025_live_seed_has9_{datetime.now().strftime('%Y-%m-%d_%H%M')}.csv",
                       mime="text/csv")
...
with tab_lab:
    ...
    # apply gate to seeds (NO9 is the only core-level gate) — keep the seed_has9 segment visible
    df["seed_has9"] = df["seed"].astype(str).str.contains("9")
    df_all = df.copy()
    df_main = df_all[~df_all["seed_has9"]].copy() if gate_no9 else df_all.copy()

    st.write(f"Total 025 hit-events (all): {len(df_all):,}")
    if gate_no9:
        st.write(f"Gated out by NO9 (seed_has9): {int(df_all['seed_has9'].sum()):,}")
        st.write(f"Events used in MAIN scoring table: {len(df_main):,}")
    else:
        st.write(f"Events used in MAIN scoring table: {len(df_main):,}")

    # Score each seed and compare to actual member (score ALL, then split for display)
    lab_rows = []
    ...
    "SeedDate": r["seed_date"].date() if pd.notna(r["seed_date"]) else None,
    "Seed": r["seed"],
    "WinnerDate": r["winner_date"].date() if pd.notna(r["winner_date"]) else None,

    "ActualWinnerMember": r["winner_member"],
    "Top1": s["top1"],
    "Top2": s["top2"],
    "Top3": s["top3"],

    # Per-member base scores (auditable)
    "Score_0025": float(s["scores"].get("0025", 0.0)),
    "Score_0225": float(s["scores"].get("0225", 0.0)),
    "Score_0255": float(s["scores"].get("0255", 0.0)),
    "Top1Score": float(s["scores"].get(s["top1"], 0.0)),
    ...
    "ForcedPick": s["forced_pick"] if s["forced_pick"] else "",
    "SeedSum": int(s["features"]["seed_sum"]),
    "SeedAbsDiff": int(s["features"]["seed_absdiff"]),
    "SeedSpread": int(s["features"]["seed_spread"]),
    "SeedHas9": int(bool(s["features"].get("seed_has_9", False))),
    "WorstPair025": int(s["features"]["seed_has_worstpair_025"]),
})

    lab_all = pd.DataFrame(lab_rows)

    # Split seed_has9 segment for visibility (does not affect MAIN metrics when gate_no9 is ON)
    if "SeedHas9" in lab_all.columns:
        lab_has9 = lab_all[lab_all["SeedHas9"].astype(int) == 1].copy()
        lab = lab_all[lab_all["SeedHas9"].astype(int) == 0].copy() if gate_no9 else lab_all.copy()
    else:
        lab_has9 = lab_all.iloc[0:0].copy()
        lab = lab_all.copy()

    if lab.empty:
        st.warning("No 025 hit-events found under current settings.")
    else:
        # Add the same "visual support" fields used in LIVE
        lab["FlipRec"] = ((lab["BaseGap(#1-#2)"] == 0) & (lab["TieFired"] == 1) & (lab["DeadTie"] == 0)).astype(int)
        _lab_gap12 = pd.to_numeric(lab.get("Gap12", 0), errors="coerce").fillna(0.0)
        _lab_seedhas9 = pd.to_numeric(lab.get("SeedHas9", 0), errors="coerce").fillna(0).astype(int)

        lab["Top1OnlySupported"] = (
            (lab["FlipRec"].astype(int) == 0) &
            (lab["DeadTie"].astype(int) == 0) &
            (lab["TieFired"].astype(int) == 0) &
            (_lab_gap12 >= float(support_gap12)) &
            (_lab_seedhas9 == 0)
        ).astype(int)

        _lab_gap_reason = f"gap12<{support_gap12:.1f}"
        lab["SupportReason"] = np.select(
            [
                (_lab_seedhas9 == 1),
                (lab["DeadTie"].astype(int) == 1),
                (lab["FlipRec"].astype(int) == 1),
                (lab["TieFired"].astype(int) == 1),
                (_lab_gap12 < float(support_gap12)),
                (lab["Top1OnlySupported"].astype(int) == 1),
            ],
            [
                "seed_has9",
                "dead_tie",
