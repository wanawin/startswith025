...
def _wf_download_name(kind: str, payload: dict | None, rules_meta: dict | None, ext: str = "csv") -> str:
    ...
    play = payload.get("play_date") or ""
    cap = payload.get("cap_streams")
    sel = payload.get("selection_mode") or ""

    if sel == "bins":
        bin_size = payload.get("bin_size_pct")
        n_bins = len(payload.get("selected_bins") or [])
        sel_tok = f"bins{bin_size}p_{n_bins}bins"
    elif sel == "rows":
        ranks = payload.get("selected_ranks") or []
        sel_tok = f"rows{len(ranks)}_{min(ranks)}to{max(ranks)}" if ranks else "rows"
    else:
        sel_tok = sel or "sel"

    w8 = ((rules_meta.get("weights") or {}).get("sha256") or "")[:8]
    t8 = ((rules_meta.get("tie_pack") or {}).get("sha256") or "")[:8]
    rules_tok = f"w{w8}_t{t8}" if (w8 or t8) else ""

    run_id = (payload.get("sig") or "")[:8]

    parts = [
        "core025_wf",
        _safe_fname_component(gate),
        _safe_fname_component(sel_tok),
        f"cap{cap}" if cap is not None else "",
        _safe_fname_component(start),
        _safe_fname_component(end),
        f"play{_safe_fname_component(play)}" if play else "",
        _safe_fname_component(rules_tok) if rules_tok else "",
        run_id,
