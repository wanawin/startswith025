...
with st.sidebar:
    ...
    # Defaults are set after history loads; you can override here.
    user_start_date = st.date_input("History start date (lookback begins)", value=None, key="wf_start")
    user_end_date   = st.date_input("History end date (last results BEFORE play date)", value=None, key="wf_end")
    user_play_date  = st.date_input("Play date (prediction target day)", value=None, key="wf_play")
...
_default_play = (_default_end + pd.Timedelta(days=1)).date()

# Initialize date inputs once if blank
if st.session_state.get("wf_start") is None:
    st.session_state["wf_start"] = _default_start
if st.session_state.get("wf_end") is None:
    st.session_state["wf_end"] = _default_end
if st.session_state.get("wf_play") is None:
    st.session_state["wf_play"] = _default_play

wf_start = st.session_state["wf_start"]
wf_end   = st.session_state["wf_end"]
wf_play  = st.session_state["wf_play"]
