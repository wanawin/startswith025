...
for c in ["PlayPick", "PlayMember", "FlipRec"]:
    if c in _cols:
        _cols.remove(c)
# Insert after PredictedMember (or after Seed if that ever changes)
_after = "PredictedMember" if "PredictedMember" in _cols else ("Seed" if "Seed" in _cols else _cols[0])
_i = _cols.index(_after) + 1
...
def _style_flip(df):
    ...
    def _row_style(row):
        if int(row.get("FlipRec", 0)) == 1:
            return ["font-weight: 700" if c in ("PlayPick", "PlayMember", "Top2") else "" for c in row.index]
