...
# ---- Calibration / Mining (scikit-learn) ----
...
def _calibrate_streamscore_logreg(dfm: pd.DataFrame, frac_train: float = 0.8, C: float = 1.0, class_weight="balanced") -> dict:
    dfm = _add_stream_history_features(dfm)
    train_mask, test_mask = _time_split_mask(dfm, frac_train=frac_train)

    rows = dfm.to_dict(orient="records")
    X_dict = [_make_streamscore_feature_dict(r) for r in rows]
    y = dfm["is_025"].astype(int).values
    ...
    out = {"model": model, "vectorizer": vec, "frac_train": frac_train}

    if test_mask.sum() > 0 and len(np.unique(y[test_mask.values])) > 1:
        proba_test = model.predict_proba(X[test_mask.values])[:, 1]
        out["test_auc"] = float(roc_auc_score(y[test_mask.values], proba_test))
        out["test_ap"] = float(average_precision_score(y[test_mask.values], proba_test))
    else:
        out["test_auc"] = np.nan
        out["test_ap"] = np.nan
    ...
    wt = pd.DataFrame({"feature": feature_names, "weight": coef}).sort_values("weight", ascending=False)
    out["weights_df"] = wt.reset_index(drop=True)
...
with tab_live:
    ...
    for _, r in latest.iterrows():
        ...
        if use_calibrated_streamscore and st.session_state.get("calib_streamscore") is not None:
            ...
            X = calib["vectorizer"].transform(Xd)
            streamprob = float(calib["model"].predict_proba(X)[:, 1][0])
