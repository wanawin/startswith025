#!/usr/bin/env python3
# BUILD: core025_master_goal_lab__2026-04-18_v4_decision_controlled_defaults
...
BUILD_MARKER = "BUILD: core025_master_goal_lab__2026-04-18_v4_decision_controlled_defaults"
APP_VERSION_STR = "core025_master_goal_lab__2026-04-18_v4_decision_controlled_defaults"
...
def apply_decision_control(df: pd.DataFrame, top2_gate_margin: float = 0.30, top2_gate_ratio: float = 0.92) -> pd.DataFrame:
    out = df.copy()
    top2_flags, why = [], []
    for _, row in out.iterrows():
        ...
        tie_fired = int(row.get("TieFired",0) or 0); correction_fired = int(row.get("CorrectionFired",0) or 0)
        ...
        if correction_fired == 1 and margin <= 0.60: trigger = True; reason.append("correction_guard")
...
def run_walkforward_lab(events_df: pd.DataFrame, overlay_rules, tie_df, correction_rules, top_n, recent_n, tie_margin_threshold, top2_gate_margin, top2_gate_ratio):
...
st.warning("Defaults are set for practical use. DAILY and LAB are separated. Nothing runs until you click a Run button.")
