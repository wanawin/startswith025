...
def normalize_history(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"History file missing columns: {missing}. Need: {needed}")
...
def load_rules(weights_csv: str, tie_csv: str, max_rules: int, min_support: int) -> Tuple[List[Rule], List[Rule]]:
    w = pd.read_csv(weights_csv)
    t = pd.read_csv(tie_csv)

    # base rules: keep only those with new_weight > 0 and support >= min_support
    w = w.copy()
    w["n_hits_gate"] = pd.to_numeric(w.get("n_hits_gate", 0), errors="coerce").fillna(0).astype(int)
    w["new_weight"] = pd.to_numeric(w.get("new_weight", 0), errors="coerce").fillna(0)

    w = w[(w["new_weight"] > 0) & (w["n_hits_gate"] >= min_support)].copy()

    # strongest first by delta_vs_baseline if present, else by support
    if "delta_vs_baseline" in w.columns:
        w["delta_vs_baseline"] = pd.to_numeric(w["delta_vs_baseline"], errors="coerce").fillna(0)
        w = w.sort_values(["delta_vs_baseline","n_hits_gate"], ascending=[False, False])
    else:
        w = w.sort_values(["n_hits_gate"], ascending=False)
    ...
    for _, r in w.iterrows():
        base_rules.append(Rule(
            ...
            op=str(r["op"]),
            value=(int(r["value"]) if str(r["value"]).isdigit() else (True if str(r["value"])=="True" else (False if str(r["value"])=="False" else r["value"]))),
            pick=str(r["pick"]).zfill(3) if len(str(r["pick"]))==3 else str(r["pick"]),
            weight=float(r["new_weight"]),
            source="base"
        ))

    tie_rules: List[Rule] = []
    for _, r in t.iterrows():
        tie_rules.append(Rule(
            feature=str(r["feature"]),
            op=str(r["op"]),
            value=(int(r["value"]) if str(r["value"]).isdigit() else (True if str(r["value"])=="True" else (False if str(r["value"])=="False" else r["value"]))),
            pick=str(r["pick"]).zfill(3) if len(str(r["pick"]))==3 else str(r["pick"]),
    ...
    # Normalize pick labels to your three members
    def norm_pick(p: str) -> str:
        # picks in csv may be 25/225/255 — normalize
        p = str(p)
        if p in ("25", "0025", "025"): return "0025"
        if p in ("225", "0225"): return "0225"
        if p in ("255", "0255"): return "0255"
        return p
    base_rules = [Rule(r.feature, r.op, r.value, norm_pick(r.pick), r.weight, r.source) for r in base_rules]
    tie_rules  = [Rule(r.feature, r.op, r.value, norm_pick(r.pick), r.weight, r.source) for r in tie_rules]
...
# Load history
try:
    # Read CSV or TXT (auto-detect delimiter for TXT)
    filename = getattr(hist_file, "name", "").lower()
    if filename.endswith(".txt"):
        # Try to auto-detect delimiter (tab, pipe, comma, etc.)
        try:
            raw = pd.read_csv(hist_file, sep=None, engine="python")
        except Exception:
            # Fallbacks
            hist_file.seek(0)
            try:
                raw = pd.read_csv(hist_file, sep="\t", engine="python")
            except Exception:
                hist_file.seek(0)
                raw = pd.read_csv(hist_file)  # last resort
    else:
...
try:
    base_rules, tie_rules = load_rules(weights_csv, tie_csv, max_rules=max_rules, min_support=min_support)
except Exception as e:
