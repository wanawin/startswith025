...
def _read_txt(f):
    ...
    # Heuristic: header row if it includes 'date' and 'state' and 'game' and 'result'
    headerish = bool(re.search(r"\bdate\b", first, re.I) and re.search(r"\bstate\b", first, re.I) and re.search(r"\bgame\b", first, re.I) and re.search(r"\bresult", first, re.I))
    ...
    if headerish:
        ...
        # Headerless TSV/TXT with 4 columns
        try:
            return pd.read_csv(
            ...
            f.seek(0)
            # last resort: auto-detect delimiter, headerless
...
if filename.endswith(".txt") or filename.endswith(".tsv"):
    ...
    if len(hist) == 0:
        st.error("No valid history rows were parsed after normalization. This usually means the TXT/TSV has broken/extra lines or the date/result formats are not being recognized.")
        st.caption("Tip: Your file should be tab-delimited with 4 columns per row: date, state, game, result (e.g., 'Thu, Mar 14, 2024\tMissouri\tPick 4 Midday\t9-3-1-9').")
