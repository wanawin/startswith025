...
def stream_score_row(seed_score: Dict[str, object], cadence_row: Dict[str, object],
                     *, w_match: float, w_gap: float, w_roll: float, w_since: float,
                     cooldown_k: int, cooldown_penalty: float,
                     score_version: str = "v1: due (higher draws-since = slightly better)",
                     since_cap: int = 50) -> float:
    """Compute StreamScore from seed-level scores + cadence context.

    NOTE: cadence_row keys can be snake_case (walk-forward) or TitleCase (export/merge).
    This function accepts BOTH, so cadence terms actually influence StreamScore.
    """

    match_strength = float(seed_score.get("match_strength", 0.0) or 0.0)
    base_gap = float(seed_score.get("base_gap", 0.0) or 0.0)

    # Robust cadence key lookup
    roll30 = float(
        cadence_row.get("rolling_025_30",
            cadence_row.get("Rolling025_30",
                cadence_row.get("CadenceRolling025_30", 0)
            )
        ) or 0.0
    )

    draws_since = float(
        cadence_row.get("draws_since_last_025",
            cadence_row.get("DrawsSinceLast025",
                cadence_row.get("CadenceDrawsSinceLast025", 9999)
            )
        ) or 9999.0
    )

    cooldown_after_hit = float(
        cadence_row.get("cooldown_after_hit",
            cadence_row.get("CooldownAfterHit", 9999)
        ) or 9999.0
    )

    # normalize cadence terms
    roll_norm = min(roll30, 30.0) / 30.0

    cap = float(max(10, int(since_cap)))
    x = min(draws_since, cap) / cap  # 0..1
    if str(score_version).startswith("v2"):
        since_term = 1.0 - x  # RECENT (smaller since) => larger term
    else:
        since_term = x        # DUE (larger since) => larger term

    ck = float(max(1, int(cooldown_k)))
    cooldown_norm = max(0.0, 1.0 - (min(cooldown_after_hit, ck) / ck))

    ss = (
        w_match * match_strength +
        w_gap * base_gap +
        w_roll * roll_norm +
        w_since * since_term -
        cooldown_penalty * cooldown_norm
    )
    return float(ss)
...
with tab_live:
    ...
    for _, r in latest.iterrows():
        ...
        , score_version=score_version, since_cap=since_cap)
