...
def _reorder_member_order_for_row_memory(member_order: List[object], row_profile: Optional[Dict[str, object]]) -> Tuple[List[str], str]:
    order = [normalize_member_id(x) for x in member_order if normalize_member_id(x) in _CORE_MEMBER_KEYS]
    while len(order) < 3:
    ...
    if not row_profile:
        return order[:3], ""
    last_member = normalize_member_id(row_profile.get("last_member"))
    actions: List[str] = []
    # Approved live rule: on strong-switch rows where the last same-row winner was 25,
    # demote 25 behind the non-25 members (no extra plays).
    if row_profile.get("strong_switch") and last_member == "0025" and "0025" in order:
        new_order = [m for m in order if m != "0025"] + ["0025"]
        if new_order != order:
            order = new_order
            actions.append("SW25_NEG")
    # Proven positive: on strong-repeat rows, gently promote the last winning member.
    if row_profile.get("strong_repeat") and last_member in order:
        new_order = [last_member] + [m for m in order if m != last_member]
        if new_order != order:
            order = new_order
            actions.append(f"{row_profile.get('tag','REP')}_BOOST")
    return order[:3], ",".join(actions)
...
def _apply_row_memory_context_to_ranked(
    ...
    for idx, row in out.iterrows():
        ...
        apply_here = (not no9_only) or (not _seed_has9(row))
        if apply_here:
            order, action = _reorder_member_order_for_row_memory([row.get("PredictedMember"), row.get("Top2"), row.get("Top3")], profile)
            out.at[idx, "PredictedMember"] = order[0]
            if "Top1" in out.columns:
                out.at[idx, "Top1"] = order[0]
            out.at[idx, "Top2"] = order[1]
            out.at[idx, "Top3"] = order[2]
            row_action_vals.append(action)
        else:
            row_action_vals.append("")
...
def _row_memory_meaning(row: pd.Series) -> str:
    ...
    if "SW25_NEG" in action:
        return f"Strong switch row — last winner {last_txt}; 25 downgraded"
    if "BOOST" in action and tag.startswith("REP"):
...
def _style_live_table(df: pd.DataFrame):
    def _row_style(row):
        ...
        elif "SW25_NEG" in action:
            bg = "background-color:#fff3bf;"  # gold = active strong-switch 25 negative
        elif "BOOST" in action and tag.startswith("REP"):
