...
with tab_lab:
    ...
    if lab.empty:
        ...
        if tierA_pct + tierB_pct >= 100:
            ...
            bin_size_pct = st.selectbox("Percentile bin size (row-zones)", options=[1, 2, 5, 10], index=2, key="wf_bin_size_pct")
            ...
            wf_sig_base = (
                ...
                json.dumps(weights_df.to_dict(orient="records"), sort_keys=True),
                json.dumps(tie_weights_df.to_dict(orient="records"), sort_keys=True),
            ...
            if run_btn or compare_btn:
                ...
                def _build_run_payload(label: str, pe_aug: pd.DataFrame, pd_aug: pd.DataFrame, ps_sel: pd.DataFrame, bins: list[str]):
                ...
                if compare_btn:
                    # Run Gate ON first to derive winner-heavy bins, then apply same bins to Gate OFF
                    pe_on, pd_on, ps_on = _wf_tier_compute_base(True)
                    selected_bins = _wf_suggest_bins_greedy(pe_on, ps_on, cap_streams=int(cap_streams), bin_size_pct=int(bin_size_pct))
                    ps_on_sel = _wf_select_cap50_by_bins(ps_on, selected_bins, cap_streams=int(cap_streams), bin_size_pct=int(bin_size_pct), fill_to_cap=bool(fill_to_cap))
                    pe_on_aug, pd_on_aug = _wf_augment_event_date_with_play50(pe_on, pd_on, ps_on_sel)
                    wf_runs["GateON"] = _build_run_payload("GateON", pe_on_aug, pd_on_aug, ps_on_sel, selected_bins)

                    pe_off, pd_off, ps_off = _wf_tier_compute_base(False)
                    ps_off_sel = _wf_select_cap50_by_bins(ps_off, selected_bins, cap_streams=int(cap_streams), bin_size_pct=int(bin_size_pct), fill_to_cap=bool(fill_to_cap))
                    pe_off_aug, pd_off_aug = _wf_augment_event_date_with_play50(pe_off, pd_off, ps_off_sel)
                    wf_runs["GateOFF"] = _build_run_payload("GateOFF", pe_off_aug, pd_off_aug, ps_off_sel, selected_bins)
                    ...
                    selected_bins = _wf_suggest_bins_greedy(pe_1, ps_1, cap_streams=int(cap_streams), bin_size_pct=int(bin_size_pct))
                    ps_sel = _wf_select_cap50_by_bins(ps_1, selected_bins, cap_streams=int(cap_streams), bin_size_pct=int(bin_size_pct), fill_to_cap=bool(fill_to_cap))
                    pe_aug, pd_aug = _wf_augment_event_date_with_play50(pe_1, pd_1, ps_sel)
                    wf_runs[label] = _build_run_payload(label, pe_aug, pd_aug, ps_sel, selected_bins)
                    per_event = pe_aug
                    per_date = pd_aug
                    per_stream = ps_sel
            ...
            if per_stream_c is not None and not per_stream_c.empty:
                st.download_button(
                    ...
                    data=per_stream_c.to_csv(index=False),
            ...
            if isinstance(wf_runs_cache, dict) and wf_runs_cache:
                with st.expander("Downloads (Gate ON vs OFF caches)", expanded=False):
                    ...
                    for _lbl, _cache in wf_runs_cache.items():
                        st.markdown(f"**{_lbl}** — ran_at: {_cache.get('ran_at','')}  |  cap: {_cache.get('cap_streams','')}  |  bin: {_cache.get('bin_size_pct','')}  |  bins: {', '.join(_cache.get('selected_bins', []))}")
