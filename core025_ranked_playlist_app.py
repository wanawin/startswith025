    ...
    *,
    ...
    for _, r in df_rulepack.iterrows():
        ...
        if _safe_eval_bool(r.get("condition", ""), dict(ctx)):
            ...
            try:
                eliminated.add(int(float(r.get("member_target"))))
            except Exception:
                pass
    if not eliminated:
        return member_order, fired
...
def apply_one_miss_downranks(
    *,
    ...
    for _, r in df_down.iterrows():
        try:
            mt = int(float(r.get("member_target")))
        except Exception:
            continue
        if mt != top1:
            continue
    ...
    return [top2, top1, member_order[2]], best_id
    ...
    *,
    ...
    for _, r in df_rescues.iterrows():
        ...
        if (status != "LOCKED_AUTO") and (not allow_guarded):
            continue
        try:
            when_top3 = int(float(r.get("when_top3")))
        except Exception:
            continue
        if when_top3 != top3:
            continue
...
def apply_mined_member_layers(
    *,
    ...
    top1 = int(s_base["top1"])
    top2 = int(s_base["top2"])
    top3 = int(s_base["top3"])
    ...
    ctx["Top1"], ctx["Top2"], ctx["Top3_infer"] = member_order[0], member_order[1], member_order[2]
...
class Rule:
    ...
    def from_row(row, source: str = "") -> "Rule":
        ...
        pick = _get("pick") or _get("member") or _get("target") or ""
        rid = _get("rule_id") or _get("id")
        w = _get("weight", _get("w", 0.0))
        try:
            w = float(w) if w is not None and str(w).strip() != "" else 0.0
        except Exception:
            w = 0.0

        return Rule(feature=str(feature).strip(), op=str(op).strip(), value=value, pick=str(pick).strip(), weight=w, source=source, rule_id=str(rid).strip() if rid is not None else None)
...
def _build_transitions_from_history(hist_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["is_025"] = df["member"].isin(["25", "225", "255"]).astype(int)
...
try:
    base_rules, tie_rules, rules_meta = load_rules(weights_csv, tie_csv, max_rules=max_rules, min_support=min_support, weights_file_up=weights_file_up, tie_up=tiepack_file_up)
except Exception as e:
...
with st.expander("Loaded Files (source / filename / sha256 / row count)"):
    try:
        ...
        def _fname_from_meta(meta_obj, path_str, up_obj, embedded_name):
            src = (meta_obj or {}).get("source")
            if src == "upload" and up_obj is not None and hasattr(up_obj, "name"):
                return up_obj.name
        ...
        rp_meta = _pack_meta_from_upload_or_embedded(rulepack_up, _EMBED_RULEPACK_V3_2_B64_GZ, "rulepack_v3_2")
        dr_meta = _pack_meta_from_upload_or_embedded(downranks_up, _EMBED_1MISS_DOWNRANKS_V1_1_B64_GZ, "1miss_downranks_v1_1")
        rr_meta = _pack_meta_from_upload_or_embedded(rescues_up, _EMBED_RESCUES_V2_B64_GZ, "rescues_v2")
...
with tab_live:
    ...
    _tmp_hits["is_025"] = (_tmp_hits["member"].isin(["25","225","255"])).astype(int)
...
with tab_lab:
    ...
    _daily_df["is_025"] = (_daily_df["member"].isin(["25", "225", "255"])).astype(int)
    ...
    for _, r in df.iterrows():
        ...
        actual = r["member"]
        hit1 = int(s_mined["top1_final"] == actual)
        hit2 = int(actual in {s_mined["top1_final"], s_mined["top2_final"]})
